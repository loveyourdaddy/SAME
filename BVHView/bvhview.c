...
static bool BVHParseNewline(Parser* par)
{
    ...
    // Skip rest of line (e.g. comments like "#name: BoneName")
    while (ParserPeek(par) != '\0' && ParserPeek(par) != '\n')
    {
        ParserInc(par);
    }
...
typedef struct {
    ...
    int renderFrame;
    FILE* ffmpegPipe;
    char renderOutputPath[512];
    bool autoRender;   // --render flag: start rendering immediately and exit when done
    bool shouldClose;  // set true to exit the main loop cleanly
...
static void ApplicationUpdate(void* voidApplicationState)
{
    ...
    if (app->isRendering)
    {
        // During rendering, drive playTime frame-by-frame
        float frameTime = (app->characterData.count > 0) ?
            app->characterData.bvhData[app->characterData.active].frameTime : (1.0f / 24.0f);
        app->scrubberSettings.playTime = app->renderFrame * frameTime;
    }
    ...
    if (app->renderSettings.drawUI)
    {
        ...
        if (app->characterData.count > 0)
        {
            if (app->isRendering)
            {
                int totalFrames = app->scrubberSettings.frameMax - app->scrubberSettings.frameMin + 1;
                DrawText(TextFormat("Rendering... %d / %d", app->renderFrame - app->scrubberSettings.frameMin + 1, totalFrames),
                    app->screenWidth / 2 - 80, app->screenHeight - 125, 15, RED);
            }
            else if (GuiButton((Rectangle){ app->screenWidth / 2 - 60, app->screenHeight - 125, 120, 20 }, "Render MP4"))
            {
                float frameTime = app->characterData.bvhData[app->characterData.active].frameTime;
                int fps = (int)(1.0f / frameTime + 0.5f);
                if (fps <= 0) fps = 24;

                // Build output path next to BVH file: replace extension with .mp4
                const char* bvhPath = app->characterData.filePaths[app->characterData.active];
                snprintf(app->renderOutputPath, sizeof(app->renderOutputPath), "%s", bvhPath);
                int len = strlen(app->renderOutputPath);
                // Replace .bvh with .mp4
                if (len > 4 && strcmp(app->renderOutputPath + len - 4, ".bvh") == 0)
                {
                    strcpy(app->renderOutputPath + len - 4, ".mp4");
                }
                else
                {
                    strcat(app->renderOutputPath, ".mp4");
                }
                ...
                snprintf(cmd, sizeof(cmd),
                    ...
                    "-pix_fmt yuv420p \"%s\"",
                    app->screenWidth, app->screenHeight, fps, app->renderOutputPath);

                app->ffmpegPipe = popen(cmd, "w");
                if (app->ffmpegPipe)
                {
                    app->isRendering = true;
                    app->renderFrame = app->scrubberSettings.frameMin;
                    app->scrubberSettings.playing = false;
                    printf("INFO: Render started -> %s  (fps=%d)\n", app->renderOutputPath, fps);
                }
                else
                {
                    snprintf(app->errMsg, sizeof(app->errMsg), "Error: Could not launch ffmpeg. Is it installed?");
                }
            }
        }
    ...
    // MP4 Frame Capture
    if (app->isRendering && app->ffmpegPipe)
    {
        Image img = LoadImageFromScreen();
        fwrite(img.data, 1, img.width * img.height * 4, app->ffmpegPipe);
        UnloadImage(img);

        app->renderFrame++;
        if (app->renderFrame > app->scrubberSettings.frameMax)
        {
            pclose(app->ffmpegPipe);
            app->ffmpegPipe = NULL;
            app->isRendering = false;
            printf("INFO: Render complete -> %s\n", app->renderOutputPath);
...
int main(int argc, char** argv)
{
    ...
    app.renderFrame = 0;
    app.ffmpegPipe = NULL;
    app.renderOutputPath[0] = '\0';
    app.autoRender = ArgBool(argc, argv, "render", false);
    ...
    // Auto-render mode: start rendering immediately
    if (app.autoRender && app.characterData.count > 0)
    {
        float frameTime = app.characterData.bvhData[app.characterData.active].frameTime;
        int fps = (int)(1.0f / frameTime + 0.5f);
        if (fps <= 0) fps = 24;
        ...
        if (app.ffmpegPipe)
        {
            ...
            app.renderFrame = app.scrubberSettings.frameMin;
            app.scrubberSettings.playing = false;
            printf("INFO: Auto-render started -> %s  (fps=%d)\n", app.renderOutputPath, fps);
        }
        else
        {
            fprintf(stderr, "ERROR: Could not launch ffmpeg\n");
        }
    }
